...
public:
    ...
    {
        const size_t matchSize = pEnd - pBegin;
        if (spotSelectorMatch[matchSize] != 0)
        {
            ...
            do
            {
                if (it->daChar == pBegin[it->pos] )
                {
                    ...
                    {
                        matchIndex = it->matchingStringIdx;
                        return true;
                    }
                }
...
private:
    void solveMatchers(
        const std::vector < std::pair < size_t, std::string > > &   in_curSizeMatches,
        const size_t &                                              curStringSize,
        std::vector < MatcherSpot > &                               in_curMatcher);

    static const size_t HOLY_GRAIL_SIZE = 64;

    char padding0[HOLY_GRAIL_SIZE]; // protect matcher from false sharing
    size_t maxKeywordSize;
    std::map < size_t, std::vector < MatcherSpot > * > spotSelector;
    std::vector < MatcherSpot > * spotSelectorMatch[256];
    std::vector < std::string > keywords;
    std::map < std::string, size_t > reverseMatchKeywords;
